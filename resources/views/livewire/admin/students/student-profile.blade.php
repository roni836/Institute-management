<div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
    {{-- Loader --}}
    <div wire:loading wire:target="updateTab" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
    ...
    <div class="bg-white rounded-2xl border shadow-sm overflow-hidden">
        <div class="md:flex items-center p-6 sm:p-8">
            <div class="h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-2xl sm:text-3xl font-bold text-white shadow-md">
            ...
            <div class="ml-0 mt-4 md:mt-0 md:ml-6 flex-1">
                ...
                <div class="mt-3">
                    <span class="px-3 py-1 rounded-full text-xs font-medium
                        @class(['bg-green-100 text-green-800' => $student->status === 'active', 'bg-gray-100 text-gray-800'])">
    ...
    <div class="bg-white rounded-2xl border shadow-sm overflow-hidden">
                ...
                @foreach(['overview', 'courses', 'payments', 'performance'] as $tab)
                    <button wire:click.debounce.500ms="updateTab('{{ $tab }}')"
                        class="px-4 py-3 text-sm font-medium border-b-2 transition-colors
                            @class(['border-blue-500 text-blue-600' => $selectedTab === $tab, 'border-transparent text-gray-500 hover:text-blue-600 hover:border-blue-200'])"
        ...
        <div class="p-4 sm:p-6" wire:loading.remove wire:target="updateTab" x-data="{ show: false }" x-init="setTimeout(() => show = true, 100)" x-show="show" x-transition.opacity>
                                    ...
                                    <div class="border rounded-xl p-4">
                                        <div class="flex justify-between items-center mb-3">
                                            ...
                                            <span class="px-2 py-1 rounded-full text-xs font-medium
                                                @class(['bg-green-100 text-green-800' => $batch['status'] === 'active', 'bg-blue-100 text-blue-800' => $batch['status'] === 'completed'])">
                                        ...
                                        <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                            <div class="h-full bg-blue-500 rounded-full transition-all duration-300" style="width: {{ $batch['progress'] }}%"></div>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Date</th>
                                        <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Batch</th>
                                        <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Amount</th>
                                        <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Mode</th>
                                        <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                                        <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Reference</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse($stats['paymentHistory']['transactions'] as $payment)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $payment['date'] }}</td>
                                            <td class="px-4 sm:px-6 py-4 text-sm text-gray-700">{{ $payment['batch'] }}</td>
                                            <td class="px-4 sm:px-6 py-4 text-sm text-gray-700">₹{{ number_format($payment['amount'], 2) }}</td>
                                            <td class="px-4 sm:px-6 py-4 text-sm capitalize text-gray-700">{{ $payment['mode'] }}</td>
                                            ...
                                            <td class="px-4 sm:px-6 py-4 text-sm text-gray-700">{{ $payment['reference_no'] ?? '-' }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                            ...
                            <div class="mt-6 space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t">
                                    <div class="text-center p-4 bg-gray-50 rounded-xl">
                                        <p class="text-sm text-gray-500">Attendance</p>
                                        <p class="text-lg font-semibold mt-1 text-gray-900">{{ $course['attendance'] }}%</p>
                                    </div>
                                    <div class="text-center p-4 bg-gray-50 rounded-xl">
                                        <p class="text-sm text-gray-500">Fee Paid</p>
                                        <p class="text-lg font-semibold mt-1 text-gray-900">
                                            ₹{{ number_format($course['fee_paid']) }}
                                            <span class="text-xs text-gray-400">/ ₹{{ number_format($course['fee_total']) }}</span>
                                        </p>
                                    </div>
                                    <div class="text-center p-4 bg-gray-50 rounded-xl">
                                        <p class="text-sm text-gray-500">Status</p>
                                        <p class="text-lg font-semibold mt-1 text-gray-900">{{ ucfirst($course['status']) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="w-32 sm:w-48 bg-gray-100 rounded-full h-2">
                                        <div class="bg-blue-500 h-2 rounded-full transition-all duration-300" style="width: {{ $performanceStats['overall']['attendance'] }}%"></div>
                                    </div>
                                    <span class="text-sm font-medium text-gray-900">{{ number_format($performanceStats['overall']['attendance'], 1) }}%</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600">Course Completion</span>
                                    <div class="w-32 sm:w-48 bg-gray-100 rounded-full h-2">
                                        <div class="bg-green-500 h-2 rounded-full transition-all duration-300" style="width: {{ $performanceStats['overall']['completion'] }}%"></div>
                                    </div>
                                    <span class="text-sm font-medium text-gray-900">{{ number_format($performanceStats['overall']['completion'], 1) }}%</span>
                                </div>
                            </div>
                        </div>
                        <div class="bg-white rounded-2xl border shadow-sm p-4 sm:p-6">
                            ...
                            <div class="space-y-4">
                                @foreach($performanceStats['courses'] as $course)
                                    <div class="border rounded-xl p-4">
                                        <h4 class="text-sm font-medium text-gray-900 mb-3">{{ $course['course'] }}</h4>
                                        <div class="space-y-4">
                                            <div class="flex justify-between items-center">
                                            ...
                                            <div class="flex justify-between items-center">
                                                <span class="text-sm text-gray-600">Attendance</span>
                                                <div class="w-32 sm:w-48 bg-gray-100 rounded-full h-2">
                                                    <div class="bg-green-500 h-2 rounded-full transition-all duration-300" style="width: {{ $course['attendance'] }}%"></div>
                                                </div>
                                                <span class="text-sm font-medium text-gray-900">{{ $course['attendance'] }}%</span>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
